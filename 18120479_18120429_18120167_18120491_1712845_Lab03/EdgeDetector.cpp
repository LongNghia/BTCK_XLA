	...
	if (method == 1) {
		...
		for (int y = 0; y < destinationImage.rows; y++) {
			...
			for (int x = 0; x < destinationImage.cols; x++) {
				//prowd[0] = (int)sqrt(prowg_x[0] * prowg_x[0] + prowg_y[0] * prowg_y[0]);
				//int val = (int)sqrt(prowg_x[x] * prowg_x[x] + prowg_y[x] * prowg_y[x]);
				int val = (abs(prowg_x[x])+ abs(prowg_y[x]));
				//int val = prowg_x[x];
		...
		if (method == 2) {
			...
			for (int y = 0; y < destinationImage.rows; y++) {
				uchar* prowd = destinationImage.ptr<uchar>(y);
				float* prowg_x = gradient_x.ptr<float>(y);
				float* prowg_y = gradient_y.ptr<float>(y);
				for (int x = 0; x < destinationImage.cols; x++, prowd++, prowg_x++, prowg_y++) {
					prowd[0] = (int)sqrt(prowg_x[0] * prowg_x[0] + prowg_y[0] * prowg_y[0]);
				}
			}
		}
			...
			if (method == 3) {
				...
				for (int y = 0; y < destinationImage.rows; y++) {
					uchar* prowd = destinationImage.ptr<uchar>(y);
					float* prowl = laplace.ptr<float>(y);
